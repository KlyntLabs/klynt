...
fn map_layout_row(row: LayoutRow) -> DomainResult<TenantDesktopLayout> {
    ...
    let icon_tree = serde_json::from_value(row.icon_tree)
...
impl TenantDesktopLayoutRepository for PgTenantDesktopLayoutRepository {
    ...
    async fn upsert(
        ...
        let icon_tree = serde_json::to_value(&layout.icon_tree).map_err(|e| {
            DomainError::internal_msg(format!("failed to serialize icon_tree: {e}"))
        })?;
