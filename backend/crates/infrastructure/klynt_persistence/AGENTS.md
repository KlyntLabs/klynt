...
Infrastructure crate providing **concrete implementations** of `klynt_base` ports using PostgreSQL and Redis.
...
**DON'T** use when:
- Writing service unit tests (use `klynt_base::testkit` instead)
- Need domain types (use `klynt_domain` directly)
...
cargo nextest run --package klynt_persistence
```

## Dependencies

- `klynt_base` — Port interfaces to implement
- `klynt_config` — Configuration
- `klynt_domain` — Domain types
- `klynt_telemetry` — Tracing/instrumentation
