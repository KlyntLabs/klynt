...
/// Adapter wrapping a legacy [`klynt_domain::repositories::UserRepository`].
...
where
    T: klynt_domain::repositories::UserRepository,
{
    ...
    async fn create_pending_user(
        ...
        let user = klynt_domain::models::User {
            ...
            institution_id: None,
            status: klynt_domain::models::UserStatus::PendingVerification,
        ...
        match self
            ...
            .await
        {
            Ok(klynt_domain::repositories::CreateResult::Created) => {
                Ok(from_legacy_user_id(user_id))
            }
            Ok(klynt_domain::repositories::CreateResult::AlreadyExists(_)) => Err(
    ...
    async fn update_password(
        ...
        let legacy_ctx = to_legacy_ctx(ctx);
        let legacy_user_id = to_legacy_user_id(user_id);
        let hashed = klynt_domain::ports::HashedPassword::new(password_hash);
...
mod tests {
    ...
    struct FakeLegacyUserRepository {
        users: Mutex<HashMap<String, klynt_domain::models::User>>,
    }
    ...
    #[async_trait]
    impl klynt_domain::repositories::UserRepository for FakeLegacyUserRepository {
        async fn create_if_not_exists(
            &self,
            _ctx: &klynt_domain::ctx::Ctx,
            email: &klynt_domain::models::Email,
            user: &klynt_domain::models::User,
        ) -> Result<klynt_domain::repositories::CreateResult, klynt_domain::errors::DomainError>
        {
            let mut users = self.users.lock().unwrap();
            if users.contains_key(email.as_str()) {
                return Ok(klynt_domain::repositories::CreateResult::AlreadyExists(
                    user.clone(),
                ));
            }
        ...
        async fn find_by_email(
            ...
            _ctx: &klynt_domain::ctx::Ctx,
            email: &klynt_domain::models::Email,
        ) -> Result<Option<klynt_domain::models::User>, klynt_domain::errors::DomainError> {
        ...
        async fn find_by_id(
            &self,
            _ctx: &klynt_domain::ctx::Ctx,
            _id: klynt_domain::models::UserId,
        ) -> Result<Option<klynt_domain::models::User>, klynt_domain::errors::DomainError> {
            Ok(None)
        }
        ...
        async fn set_email_verified(
            ...
            for user in users.values_mut() {
                if user.id == user_id {
                    user.status = klynt_domain::models::UserStatus::Active;
                    return Ok(());
                }
            }
            Ok(())
        ...
        async fn update_password(
            ...
            _ctx: &klynt_domain::ctx::Ctx,
            user_id: klynt_domain::models::UserId,
            password_hash: &klynt_domain::ports::HashedPassword,
        ) -> Result<(), klynt_domain::errors::DomainError> {
            let mut users = self.users.lock().unwrap();
            for user in users.values_mut() {
                if user.id == user_id {
